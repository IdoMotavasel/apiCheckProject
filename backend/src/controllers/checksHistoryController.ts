...
export const getChecksHistory = async (req:Request<Params,{},{}>, res:Response): Promise<void> => {
    ...
    try{
        ...
        const requests: UserRequestType[] = await UserRequest.find({ username })
            .populate({
                path: "apiCodeId",
                select: "description",
            })
            .select("isValid adminsNotes adminsDecision createdAt apiCodeId")
            .exec();
